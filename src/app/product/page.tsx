...
const ProductPage = () => {
  ...
  return (
    <div className="container mx-auto flex flex-col space-y-8 p-6 mt-[124px] max-w-screen-xl">
      {/* First Section */}
      <section className="flex flex-col lg:flex-row lg:space-x-8 space-y-8 lg:space-y-0">
        ...
        <div className="lg:w-1/3 w-full space-y-4">
          <motion.h1 className="text-2xl font-semibold" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
            Product Title
          </motion.h1>
          <div className="flex flex-row gap-2 items-center">
            <motion.div className="text-xl text-yellow-500" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2, duration: 0.5 }}>
              ★★★★☆
            </motion.div>{" "}
            |
            <motion.p className="text-sm text-gray-500" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4, duration: 0.5 }}>
              XYZ
            </motion.p>
          </div>
          <motion.div className="flex  p-1  rounded-lg shadow-md" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <div className="lg:w-1/3 w-full space-y-4">
          ...
          <motion.div className="p-6 bg-gray-100 rounded-2xl shadow-lg space-y-4" whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.95 }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} >
            <div>
              {/* Original Price and Discount */}
              <div className="flex justify-between items-center">
                <p className="line-through text-gray-500">799 SAR</p>
                <p className="text-red-600 text-sm">SAVE 399 SAR</p>
              </div>

              {/* Final Price and Discount Percentage */}
              <div className="flex justify-between items-center">
                <p className="text-3xl font-semibold">400 SAR</p>
            ...
            {/* Purchase Option */}
            <div className="flex items-center space-x-2">
              <input type="radio" name="purchase-option" value="purchase-now" checked={purchaseOption === "purchase-now"} onChange={() => setPurchaseOption("purchase-now")} className="form-radio h-4 w-4 text-blue-600 transition duration-150 ease-in-out" />
              <p>Purchase now</p>
            </div>

            {/* Add to Cart Button */}
            <motion.button className="mt-4 w-full py-2 px-4 bg-green-600 text-white rounded-lg" whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.95 }}>
              Add to Cart
            </motion.button>
          </motion.div>

          {/* Payment Methods Card */}
          <motion.div className="p-6 bg-gray-100 rounded-2xl shadow-lg flex items-center space-x-4" whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.95 }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} >
            <div className="text-sm">
              <span>Pay with Madfu </span>
              <span className="font-bold text-teal-500 underline">on 6 installments</span>
              <span> with value of installment </span>
              <span className="font-bold text-teal-500 underline">66.67 SAR</span>
              <span className="text-xs">(Warning: Maximum value is 2000 SAR)</span>
            </div>
            <div>
          ...
          <motion.div className="p-6  flex-row  bg-gray-100 rounded-2xl shadow-lg relative flex items-center space-x-4 cursor-pointer font-sans text-left text-sm text-black" whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.95 }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} >
            <div className="">
              <p>
      ...
      {/* Second Section */}
      <section className="flex flex-col lg:flex-row lg:space-x-8 space-y-8 lg:space-y-0">
        <div className="w-full bg-gray-50 rounded-2xl shadow-lg">
          <div className="flex justify-around">
            <p id="dt-description" className={`cursor-pointer text-sm lg:text-lg p-3 ${activeTab === "description" ? "font-bold text-green-600" : ""}`} onClick={() => setActiveTab("description")}>
              Description
            </p>
            <p id="dt-additional-info" className={`cursor-pointer text-sm lg:text-lg p-3 ${activeTab === "additional-info" ? "font-bold text-green-600" : ""}`} onClick={() => setActiveTab("additional-info")}>
              Additional Information
            </p>
            <p id="dt-reviews" className={`cursor-pointer text-sm lg:text-lg p-3 ${activeTab === "reviews" ? "font-bold text-green-600" : ""}`} onClick={() => setActiveTab("reviews")}>
              Ratings
            </p>
          </div>
          ...
          <div className="p-6">
            {activeTab === "description" && (
              <div id="dt-description-content" className="space-y-2">
                <p>A Turkish coffee maker gives you the perfect cup of coffee filled with flavors and aromas that awaken your senses and inspire your day.</p>
                <p>
