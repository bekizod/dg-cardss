          ...
          <motion.div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }}>
            <div className="p-4">
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="text-center text-gray-600 dark:text-gray-400 mb-4">Your cart is empty. Add your favorite products to it.</div>
      ...
      {!cartEmpty && (
        <div className="flex flex-col md:flex-row gap-3 px-3 w-full">
              ...
              <div className="flex items-center justify-center w-full max-w-4xl relative">
                {steps.map((step, index) => (
                  <div key={step.id} className="relative flex flex-col items-center w-full">
                    ...
                    {/* Bottom Label */}
                    <div className={`text-sm mt-2 font-medium ${activeStep >= step.id ? "text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400"}`}>{step.name}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="mt-4 md:mt-0 p-4 bg-gray-50 dark:bg-gray-800 shadow-lg rounded-xl w-full md:w-2/4 lg:w-1/4">
            ...
            {/* Order Summary Title */}
            <div className="text-xl font-semibold dark:text-gray-200">Order Summary</div>
           ...
           <div>
             ...
             
            {
              ...
              <div className="line-through text-gray-500 dark:text-gray-400">{item.price}</div>
              <div className="text-lg font-bold text-green-600">{item.unitPrice}</div>
              
              </div>
              <div className="flex w-full justify-end">
                  <div className="text-lg font-bold">-{Math.round(item.discount)}%</div>
