...
const Page = () => {
  ...
  <div className="w-3/4 p-6 h-full overflow-y-auto scrollbar-hidden bg-white dark:bg-slate-900">
    {loading && !subCategories?.length ? (
      ...
      <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
        {subCategories.map((card) => (
          <Link href={`/${parentName}/${card.parentCategory}/${card.categoryName}/${card._id}`} key={card._id}>
            <motion.div 
              key={card._id} 
            ...
            >
              <Image src={card.categoryLogo?.data} alt={card.categoryName} width={1000} height={1000} className="w-full object-contain mb-4 h-28 rounded-md" />
