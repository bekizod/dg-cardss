...
const addresses: any[] = [
  {
    ...
    name: "tt nnt",
    details:
      "RHSC3084, 3084 Salih Al Khazraji, 7430, As Sulimaniyah, Riyadh 12232, Saudi Arabia",
  },
  {
    id: 1,
    name: "tt nnt",
        ...
        <div className="gap-4">
          ...
          {addresses.length > 0 ? (
            <div className="space-y-4">
              {addresses.map((address) => (
                <motion.div
                  key={address.id}
                  className="p-4 bg-gray-100 rounded-lg shadow-md w-full flex justify-between items-start dark:bg-gray-800"
                  initial={{ opacity: 0, y: -20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.3 }}
                >
            ...
            <p className="text-center text-gray-500 dark:text-gray-400">
              No addresses available. Add Address
            </p>
          )}
          ...
          <div className="flex justify-center mt-4">
            <Link href="/checkout1/payment">
              <motion.button
                className=" bg-[var(--color-primary)] text-white p-2 rounded-lg shadow-md hover: bg-[var(--color-primary)] transition duration-300 dark: bg-[var(--color-primary)] dark:hover:bg-green-700"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.3 }}
              >
