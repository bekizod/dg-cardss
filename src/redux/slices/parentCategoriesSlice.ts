...
export const fetchFirstFourParentCategories = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get('https://alsaifgallery.onrender.com/api/v1/category/getParentCategories?limit=4', {
        headers: {
...
const parentCategoriesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchFirstFourParentCategories.rejected, (state : any, action) => {
        state.isLoading = false;
