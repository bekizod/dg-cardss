...
export const fetchFavorites = createAsyncThunk('favorites/fetch', async () => {
  ...
  const response = await axios.get('https://alsaifgallery.onrender.com/api/v1/user/getFavoriteCategories', {
...
export const addFavorite = createAsyncThunk('favorites/add', async (categoryId: string) => {
  const token = Cookies.get('token');
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(removeFavorite.fulfilled, (state, action) => {
        state.favorites = state.favorites.filter((category) => category._id !== action.payload);
      });
  },
});
