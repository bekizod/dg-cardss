...
export const saveFavoriteProduct = createAsyncThunk(
  ...
  async (productId: string, { rejectWithValue }) => {
    try {
      const token = Cookies.get("token");
      const response = await fetch(
        "https://alsaifgallery.onrender.com/api/v1/user/saveFavoriteProducts",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
...
export const removeFavoriteProduct = createAsyncThunk(
  ...
  async (productId: string, { rejectWithValue }) => {
    try {
      const token = Cookies.get("token");
      const response = await fetch(
        `https://alsaifgallery.onrender.com/api/v1/user/removeFavoriteProduct/${productId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
...
const favoriteProductsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        saveFavoriteProduct.fulfilled,
        (
          state,
          action: PayloadAction<{ productId: string; productData: Product }>
        ) => {
          state.loading = false;
...
export const { addFavoriteLocally, removeFavoriteLocally } =
