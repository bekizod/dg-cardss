...
export const fetchThemeFromAPI = createAsyncThunk('theme/fetchTheme', async (_, { rejectWithValue }) => {
  try {
    const response = await fetch('https://alsaifgallery.onrender.com/api/v1/theme/getTheme');
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchThemeFromAPI.fulfilled, (state, action) => {
        ...
        const { primaryColor, secondaryColor, tertiaryColor, logo } = action.payload;
